...
RUN set -eux; \
		...
		apt install -y --no-install-recommends \
			curl ca-certificates gcc libc6-dev pkg-config libssl-dev libsqlite3-dev\
			;
...
RUN --mount=type=cache,target=/root/.cargo/registry \
    --mount=type=cache,target=/root/.cargo/git \
		...
		set -eux; \
		cargo build --release; \
		objcopy --compress-debug-sections ./target/release/routines ./routines
...
WORKDIR /app
COPY --from=builder /app/routines .
